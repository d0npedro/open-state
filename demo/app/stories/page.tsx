...
const domainMeta: Record<string, { label: string; hint: string; farbe: string }> = {
  'Arbeitsverwaltung': {
    label: 'Arbeitsverwaltung',
    hint: 'ALG I · Bürgersicht · Vollständig demonstrierbar',
    farbe: 'var(--color-primary)',
  },
  'Kita-Betrieb & Jugendamt-Steuerung': {
    label: 'Kita-Betrieb & Jugendamt-Steuerung',
    hint: 'Kita-Versorgung · Öffentlich + JA-intern · Teilweise demonstrierbar',
    farbe: '#2e7d32',
  },
  'Unternehmensgründung': {
...
export default function StoriesPage() {
  ...
  for (const d of domainOrder) grouped.set(d, []);
  for (const story of storyRegistry) {
    if (!grouped.has(story.domain)) grouped.set(story.domain, []);
    grouped.get(story.domain)!.push(story);
  }
      ...
      <main style={{ padding: '2rem 0' }}>
        <div className="container" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div style={{ background: 'var(--color-primary-light)', borderRadius: 'var(--radius)', padding: '1rem 1.25rem', fontSize: '0.875rem' }}>
            Diese Ansicht zeigt, welche User Stories im Demonstrator abgedeckt sind.
                ...
                <section key={domain}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {stories.map(story => {
                      ...
                      const pct = story.implemented_criteria && story.acceptance_criteria_count
                        ? Math.round((story.implemented_criteria / story.acceptance_criteria_count) * 100)
                        : sc.pct;
                      return (
