...
|---|----------|------------------|--------|------------------------|
| 0.1 | `archive/rewrites/` anlegen; alle `REPO_REWRITE_SUMMARY*.md` + `REPO_CONSOLIDATION_SUMMARY.md` dorthin verschieben; Root-README um einen Satz „Historische Umschreibungen → archive/“ ergänzen | Root `REPO_*.md` → `archive/rewrites/` | Root: 19 → ~7 MD-Dateien; klarer Einstieg | Nur verschieben, nicht löschen; relative Links in Summaries egal (Historie) |
| 0.2 | `NEXT_STEPS_QUEUE.md` spalten: aktive Queue (OFFEN/IN_ARBEIT/BLOCKIERT + max. letzte ~10 DONE) vs. `docs/delivery/queue-archive/Q-001-to-Q-NNN.md` | `docs/NEXT_STEPS_QUEUE.md`, neu `docs/…/queue-archive/` | Agenten lesen Minuten statt Scrollen | DONE-Historie mit Commit-Hashes behalten |
